...
// ─── Auth middleware ──────────────────────────────────────────────────────────
function verificarMasterKey(req, res, next) {
...
router.get('/master/ping', async (_req, res) => {
    const totalClientes = await Empresa_1.Empresa.countDocuments();
    const activos = await Empresa_1.Empresa.countDocuments({ suscripcionActiva: true, accesoBloqueado: false });
    res.json({
...
router.get('/master/clients/:id', async (req, res) => {
    try {
        ...
        const usuarios = await Usuario_1.Usuario.find({ empresaId: empresa._id }, 'nombre email rol activo createdAt').lean();
...
router.put('/master/clients/:id/subscription', async (req, res) => {
    try {
        const { activa } = req.body;
        if (typeof activa !== 'boolean')
            return res.status(400).json({ error: 'Campo "activa" booleano requerido' });
        const empresa = await Empresa_1.Empresa.findByIdAndUpdate(req.params.id, { suscripcionActiva: activa }, { new: true });
        if (!empresa)
            return res.status(404).json({ error: 'Cliente no encontrado' });
...
router.put('/master/clients/:id/access', async (req, res) => {
    try {
        const { bloqueado } = req.body;
        if (typeof bloqueado !== 'boolean')
            return res.status(400).json({ error: 'Campo "bloqueado" booleano requerido' });
        const empresa = await Empresa_1.Empresa.findByIdAndUpdate(req.params.id, { accesoBloqueado: bloqueado }, { new: true });
        if (!empresa)
            return res.status(404).json({ error: 'Cliente no encontrado' });
...
router.put('/master/clients/:id/plan', async (req, res) => {
    try {
        const { plan } = req.body;
