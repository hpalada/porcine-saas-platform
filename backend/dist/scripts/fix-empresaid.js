...
async function run() {
    ...
    for (const l of lotes) {
        if (l.empresaId)
            loteMap.set(l._id.toString(), l.empresaId);
    }
    ...
    if (remainingGastos.length > 0) {
        ...
        if (empresaConMasLotes[0]) {
            await db.collection('gastoadicionals').updateMany({ empresaId: { $exists: false } }, { $set: { empresaId: empresaConMasLotes[0]._id } });
    ...
    if (remainingConcentrados.length > 0) {
        ...
        if (empresaConMasLotes[0]) {
            ...
            await db.collection('concentradotipos').updateMany({ empresaId: { $exists: false } }, { $set: { empresaId: defaultEmpresaId } });
    ...
    for (const c of allConcentrados)
        concentradoMap.set(c._id.toString(), c.empresaId);
    ...
    if (remainingMovs.length > 0) {
        ...
        if (empresaConMasLotes[0]) {
            await db.collection('inventariomovimientos').updateMany({ empresaId: { $exists: false } }, { $set: { empresaId: empresaConMasLotes[0]._id } });
    ...
    for (const v of ventasOldSchema) {
        await db.collection('ventaregistros').updateOne({ _id: v._id }, { $rename: { pesoTotal: 'pesoTotalKg' }, $set: { unidadPeso: 'kg' } });
    }
...
//# sourceMappingURL=fix-empresaid.js.map