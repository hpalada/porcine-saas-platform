...
async function run() {
    ...
    const loteActivo = lotes.find((l) => (l.cantidadActual ?? 0) > 0);
    if (!loteActivo) {
        ...
        await db.collection('lotes').updateOne({ _id: loteActivo._id }, { $set: { cantidadSalida: nuevaSalida, cantidadActual: nuevoActivo, estado: nuevoEstado } });
        ...
        // Revertir
        await db.collection('lotes').updateOne({ _id: loteActivo._id }, { $set: { cantidadSalida: loteActivo.cantidadSalida, cantidadActual: loteActivo.cantidadActual, estado: loteActivo.estado } });
...
//# sourceMappingURL=test-ventas-flow.js.map