...
exports.mortalidadController = {
    ...
    async crear(req, res) {
        ...
        try {
            ...
            const lote = await Lote_1.Lote.findOne({ _id: loteId, empresaId }).session(session);
            if (!lote)
            ...
            const mortalidad = new MortalidadLote_1.default({
                empresaId,
                loteId,
    ...
    async actualizar(req, res) {
        ...
        try {
            ...
            const mortalidad = await MortalidadLote_1.default.findOneAndUpdate({ _id: req.params.id, empresaId }, updateData, { new: true, runValidators: true, session });
            if (!mortalidad)
            ...
            if (updateData.cantidadMuertas !== undefined) {
                const oldMortalidad = await MortalidadLote_1.default.findById(req.params.id).session(session);
                if (!oldMortalidad) {
                ...
                const lote = await Lote_1.Lote.findOne({ _id: oldMortalidad.loteId, empresaId }).session(session);
                if (lote) {
    ...
    async eliminar(req, res) {
        ...
        try {
            const empresaId = getEmpresaId(req);
            const mortalidad = await MortalidadLote_1.default.findOneAndDelete({ _id: req.params.id, empresaId }).session(session);
            if (!mortalidad)
                return res.status(404).json({ error: 'Mortalidad no encontrada' });
            // Reverse the mortality: add the cantidadMuertas back to the lote
            const lote = await Lote_1.Lote.findOne({ _id: mortalidad.loteId, empresaId }).session(session);
            if (lote) {
