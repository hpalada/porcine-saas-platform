...
export const ventasController = {
  async index(req: Request, res: Response) {
    try {
      ...
      if (fechaDesde || fechaHasta) {
        query.fechaVenta = {};
        if (fechaDesde) query.fechaVenta.$gte = new Date(fechaDesde as string);
        if (fechaHasta) query.fechaVenta.$lte = new Date(fechaHasta as string);
      }
      const ventas = await VentaRegistro.find(query)
  ...
  async show(req: Request, res: Response) {
    try {
      const empresaId = getEmpresaId(req);
      const venta = await VentaRegistro.findOne({ _id: req.params.id, empresaId }).populate('loteId', 'nombre');
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      const existing = await VentaRegistro.findOne({ _id: req.params.id, empresaId });
      ...
      if (cantidadCerdos !== undefined) {
        ...
        if (diff !== 0) {
          const lote = await Lote.findOne({ _id: existing.loteId, empresaId });
          if (lote) {
      ...
      if (precioPorKg !== undefined) updateData.precioPorKg = precioPorKg;
      if (precioPorLb !== undefined) updateData.precioPorKg = precioPorLb / KG_PER_LB;
      ...
      const venta = await VentaRegistro.findOneAndUpdate({ _id: req.params.id, empresaId }, updateData, { new: true, runValidators: true });
      if (!venta) return res.status(404).json({ error: 'Venta no encontrada' });
      res.json(venta);
    } catch (error) {
  ...
  async delete(req: Request, res: Response) {
    try {
      const empresaId = getEmpresaId(req);
      const venta = await VentaRegistro.findOneAndDelete({ _id: req.params.id, empresaId });
      if (!venta) return res.status(404).json({ error: 'Venta no encontrada' });
